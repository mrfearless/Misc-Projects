	...
	function main($id, $mode)
	{
		...
		switch($mode)
		{
			...
			case 'nwodonations':
				 ...
				 * Add Donation *
				 ****************/
				if ($submode == 'adddonation')
				{
					if ($submit)
					{
						$donateuser = (int) request_var('donateuser', 0);
						$donateemail = utf8_normalize_nfc(request_var('donateemail', ''), true);
						$donateamount = (int) request_var('donateamount', 0);
						$donatetype = (int) request_var('donatetype', 0);

						if ($donateuser != 0)
						{
							$donateusername = $usersarray[$donateuser];
						}
						else
						{
							$donateusername = 'Anonymous / Unknown';
						}

						if (empty($donateemail)) // if email is empty then find the one from the phpbb forums to use instead
						{
							$sqluser = "SELECT user_id, username, user_email FROM " . USERS_TABLE . " WHERE user_id = '$donateuser'" ;
							$resultuser = $db->sql_query($sqluser);
							if ($db->sql_affectedrows() != 0)
							{
								$rowuser = $db->sql_fetchrow($resultuser);
								$donateemail = $rowuser['user_email'];
							}
						}

						// Create new donate array for sql insert
						$sql_nwodonate = array(
							'firstname'				=> $donateusername,
							'lastname'				=> '',
							'buyer_email'			=> $donateemail,
							'street'				=> '',
							'city' 					=> '',
							'state' 				=> '',
							'zipcode'				=> '',
							'paymentdate'			=> date("H:m:s M d, Y e"),
							'paymenttype'			=> 'Manual',
							'txnid'					=> 'NWO' . strtotime(),
							'mc_gross'				=> $donateamount,
							'mc_fee'				=> '0.0',
							'paymentstatus'			=> 'Completed',
							'datecreation'			=> date("Y-m-d"),
						);
						$db->sql_query('INSERT INTO ' . NWOPAYPALPAYMENTS_TABLE . $db->sql_build_array('INSERT', $sql_nwodonate));

						// Log entry to tell everyone that a nwo donation entry has been made
						add_log('admin', 'LOG_NWOMANAGER_DONATION', !empty($donateusername) ? $donateusername : 'NWO Manual Donation Added');

						trigger_error(sprintf($user->lang['ACP_NWOMANAGER_DONATION_SAVED'], append_sid('index.php?i=' . $id . '&mode=nwodonations')));
					}
					else
					{
						$template->assign_vars(array(
							'DONATE_USER'			=> build_options_select($usersarray, 0),
							'DONATE_EMAIL'			=> '',
							'DONATE_AMOUNT'			=> 0, //build_options_select($servertypearray, 0),
							'DONATE_TYPE'			=> 0,
							'U_ACP_NWOMANAGER_DONATIONS'	=> append_sid('index.php?i=' . $id . '&mode=nwodonations'),
							'U_ACTION'				=> append_sid('index.php?i=' . $id . '&mode=nwodonations&submode=adddonation'),
							'DONATE_PAGETITLE'		=> $user->lang['ACP_NWOMANAGER_DONATION'],
							'DONATE_TITLE'			=> $user->lang['ACP_NWOMANAGER_DONATION'],
						));

						$this->tpl_name = 'acp_nwomanager_donation';
						$this->page_title = 'ACP_NWOMANAGER_DONATION';
					}
				}
				 ...
				 * Edit Donation *
				 *****************/
				elseif ($submode == 'editbanner')
				{
					if ($bannerid < 0)
					{
						trigger_error('ACP_NO_BANNER_ID');
					}

					$sqlbanner = 'SELECT *
						FROM ' . NWOBANNERIMAGES_TABLE . '
						WHERE banner_id= ' . $bannerid .'';
					$resultbanner = $db->sql_query($sqlbanner);
					$rowbanner = $db->sql_fetchrow($resultbanner);

					$bannername = utf8_normalize_nfc(request_var('bannername', $rowbanner['banner_name']), true);
					$bannerimage = utf8_normalize_nfc(request_var('bannerimage', $rowbanner['banner_image']), true);
					$bannertype = (int) request_var('bannertype', $rowbanner['banner_type']);
					$bannerlinktarget = utf8_normalize_nfc(request_var('bannerlinktarget', $rowbanner['banner_linktarget']), true);
					$bannerdescription = utf8_normalize_nfc(request_var('bannerdescription', $rowbanner['banner_description']), true);
					$bannervisible = (int) request_var('bannervisible', $rowbanner['banner_visible']);

					if ($submit)
					{
						$sql = 'UPDATE ' . NWOBANNERIMAGES_TABLE . '
							SET
							banner_name 		= "' . $bannername . '",
							banner_image 		= "' . $bannerimage . '",
							banner_type			= "' . $bannertype . '",
							banner_linktarget	= "' . $bannerlinktarget . '",
						...
						trigger_error(sprintf($user->lang['ACP_NWOMANAGER_BANNER_SAVED'], append_sid('index.php?i=' . $id . '&mode=nwobannerimages')));
					}

					$pagetitle = $user->lang['ACP_NWOMANAGER_BANNERIMAGE'] . (!empty($bannername) ? ' [' . $bannername . ']' : '');

					$template->assign_vars(array(
						'BANNER_NAME'			=> $bannername,
						'BANNER_IMAGE'			=> $bannerimage,
						'BANNER_TYPE'			=> 0, //build_options_select($servertypearray, $servertype),
						'BANNER_LINK'			=> $bannerlinktarget,
						'BANNER_DESC'			=> $bannerdescription,
						'S_BANNER_VISIBLE'		=> $bannervisible,
						'U_ACP_NWOMANAGER_BANNERIMAGES'	=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages'),
						'BANNER_PAGETITLE'		=> $pagetitle,
						'BANNER_TITLE'			=> !empty($bannername) ? $bannername : $user->lang['ACP_NWOMANAGER_BANNERIMAGE'],
					));
					$db->sql_freeresult($resultserver);

					$this->tpl_name = 'acp_nwomanager_bannerimage';
				...
				else
				{
					...
					//while($row = $db->sql_fetchrow($result))
					{
						...
						$sqluser = "SELECT username, user_email FROM " . USERS_TABLE . " WHERE user_email = '$buyeremail'" ;
						$resultuser = $db->sql_query($sqluser);
						if ($db->sql_affectedrows() != 0)
						{
							$rowuser = $db->sql_fetchrow($resultuser);
							$donoruser = $rowuser['username'];
						}
						else
						{
							$donoruser = '';
						...
						$template->assign_block_vars('transactions', array(
							'DONOR_NAME'			=> !empty($donoruser) ? $donoruser : strtoproper(trim($row['firstname'])) . ' ' . strtoproper(trim($row['lastname'])),
							'DONOR_EMAIL'			=> $buyeremail,
							'DONOR_AMOUNT'			=> $row['mc_gross'],
							'DONOR_PAYSTATUS'		=> $row['paymentstatus'],
							'DONOR_DATE'			=> $row['datecreation'],
							'DONOR_SUBMIT'			=> append_sid('index.php?i=' . $id . '&mode=nwodonations'),
							'U_ACP_NWOMANAGER_DONATIONS' => append_sid('index.php?i=' . $id . '&mode=nwodonations'),
						));
					}
					$db->sql_freeresult($result);

					$sql = 'SELECT COUNT(txnid) AS txn_count FROM ' . NWOPAYPALPAYMENTS_TABLE . ' WHERE txnid !=""';
					$result = $db->sql_query($sql);
					$total_txn = $db->sql_fetchfield('txn_count');
					$db->sql_freeresult($result);

					$action = $this->u_action;
					$template->assign_vars(array(
	    				'PAGINATION'		=> generate_pagination($action, $total_txn, $limit, $start),
		    			'PAGE_NUMBER'		=> on_page($total_txn, $limit, $start),
	    				'TOTAL_TRANSACTIONS'=> $total_txn,
					));

					$this->tpl_name = 'acp_nwomanager_donations';
					$this->page_title = 'ACP_NWOMANAGER_DONATIONS';
				}
			break;
			...
			/*********************************************************************************************************************************************************
			 * Banner images in ACP
			 *********************************************************************************************************************************************************/
			case 'nwobannerimages':
				$submode = request_var('submode', '');
				$bannerid = request_var('bannerid', '');
				$addbanner = request_var('addbanner', '');

				// Handle some specials cases first for submode. Like add banner
				if (!empty($addbanner))
				{
					$submode = 'addbanner';
				}
				 ...
				 * Add New Banner *
				 ******************/
				if ($submode == 'addbanner')
				{
					if ($submit)
					{

						$bannername = utf8_normalize_nfc(request_var('bannername', 'New Banner Image'), true);
						$bannerimage = utf8_normalize_nfc(request_var('bannerimage', ''), true);
						$bannertype = (int) request_var('bannertype', 0);
						$bannerlinktarget = utf8_normalize_nfc(request_var('bannerlinktarget', ''), true);
						$bannerdescription = utf8_normalize_nfc(request_var('bannerdescription', ''), true);
						$bannervisible = (int) request_var('bannervisible', 1);

						// Get total banners. This for the order id, new entry is added with total banner+1
						$sql = 'SELECT COUNT(banner_id) AS total_banners FROM ' . NWOBANNERIMAGES_TABLE;
						$result = $db->sql_query($sql);
						$total_banners = $db->sql_fetchfield('total_banners');
						$db->sql_freeresult($result);
						...
						$sql_nwobanner = array(
							...
							'banner_linktarget'		=> $bannerlinktarget,
							'banner_description'	=> $bannerdescription,
							'banner_order'			=> (int) $total_banners -1 +2,
							'banner_visible'		=> $bannervisible,
						);
						$db->sql_query('INSERT INTO ' . NWOBANNERIMAGES_TABLE . $db->sql_build_array('INSERT', $sql_nwobanner));
						trigger_error(sprintf($user->lang['ACP_NWOMANAGER_BANNER_SAVED'], append_sid('index.php?i=' . $id . '&mode=nwobannerimages')));
					}
					else
					{
						...
						$template->assign_vars(array(
							'BANNER_NAME'			=> $bannername,
							'BANNER_IMAGE'			=> '',
							'BANNER_TYPE'			=> 0, //build_options_select($servertypearray, 0),
							'BANNER_LINK'			=> '',
							'BANNER_DESC'			=> '',
							'S_BANNER_VISIBLE'		=> true,
							'U_ACP_NWOMANAGER_BANNERIMAGES'	=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages'),
							'U_ACTION'				=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages&submode=addbanner'),
							'BANNER_PAGETITLE'		=> $user->lang['ACP_NWOMANAGER_BANNERIMAGE'],
							'BANNER_TITLE'			=> $user->lang['ACP_NWOMANAGER_BANNERIMAGE'],
						));

						$this->tpl_name = 'acp_nwomanager_bannerimage';
						$this->page_title = 'ACP_NWOMANAGER_BANNERIMAGE';
					}
				}
				 ...
				 * Delete Banner  *
				 ******************/
				elseif ($submode == 'deletebanner')
				{
					$cancelbanner = request_var('cancelbanner', '');
					$confirmbanner = request_var('confirmbanner', '');

					if (!empty($cancelbanner))
					{
						redirect(append_sid('index.php?i=' . $id . '&mode=nwobannerimages'));
					}
					if (empty($confirmbanner))
					{
						trigger_error('ACP_CONFIRM_BANNER_DELETE');
					}
					if ($bannerid < 0)
					{
						trigger_error('ACP_NO_BANNER_ID');
					...
					$sql = 'DELETE FROM ' . NWOBANNERIMAGES_TABLE . '
							WHERE banner_id = ' . $bannerid;
					$db->sql_query($sql);
					trigger_error(sprintf($user->lang['ACP_BANNER_DELETED'], append_sid('index.php?i=' . $id . '&mode=nwobannerimages')));
				}
				 ...
				 * Move Banner Dn *
				 ******************/
				elseif ($submode == 'movebannerdn')
				{
					$sqlbanner = 'SELECT banner_order
						FROM ' . NWOBANNERIMAGES_TABLE . '
						WHERE banner_id= ' . $bannerid .'';
					$resultbanner = $db->sql_query($sqlbanner);
					$rowbanner = $db->sql_fetchrow($resultbanner);

					$bannerorder = $rowbanner['banner_order'];

					$sql = 'UPDATE ' . NWOBANNERIMAGES_TABLE . '
							SET
							banner_order = ' . $bannerorder . '
							WHERE banner_order = ' . $bannerorder . '-1 +2';
					$db->sql_query($sql);

					$sql = 'UPDATE ' . NWOBANNERIMAGES_TABLE . '
							SET
							banner_order = ' . $bannerorder . '-1 +2
							WHERE banner_id = ' . $bannerid;
					$db->sql_query($sql);
					$db->sql_freeresult($resultbanner);
				...
				elseif ($submode == 'movebannerup')
				{
					...
					$sql = 'UPDATE ' . NWOBANNERIMAGES_TABLE . '
							SET
							banner_order = ' . $bannerorder . '
							WHERE banner_order = ' . $bannerorder . '-2 +1';
					$db->sql_query($sql);

					$sql = 'UPDATE ' . NWOBANNERIMAGES_TABLE . '
							SET
							banner_order = ' . $bannerorder . '-2 +1
							WHERE banner_id = ' . $bannerid;
					$db->sql_query($sql);
					$db->sql_freeresult($resultbanner);
				 ...
				 * Toggle Banner  *
				 ******************/
				elseif ($submode == 'togglebanner')
				{
					$sqlbanner = 'SELECT banner_visible
						FROM ' . NWOBANNERIMAGES_TABLE . '
						WHERE banner_id= ' . $bannerid .'';
					$resultbanner = $db->sql_query($sqlbanner);
					$rowbanner = $db->sql_fetchrow($resultbanner);

					$bannervisible = $rowbanner['banner_visible'];
					$sql = 'UPDATE ' . NWOBANNERIMAGES_TABLE . '
							SET
							banner_visible = ' . ($bannervisible ? "0" : "1") .'
							WHERE banner_id = ' . $bannerid;
					$db->sql_query($sql);
					$db->sql_freeresult($resultbanner);
				...
				else
				{
					...
					while($row = $db->sql_fetchrow($result))
					{
						$template->assign_block_vars('banners', array(
							'U_EDIT'				=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages&submode=editbanner&bannerid=' . $row['banner_id']),
							'U_DELETE'				=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages&submode=deletebanner&bannerid=' . $row['banner_id']),
							'U_MOVE_DOWN'			=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages&submode=movebannerdn&bannerid=' . $row['banner_id']),
							'U_MOVE_UP'				=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages&submode=movebannerup&bannerid=' . $row['banner_id']),
							'U_VISIBLE'				=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages&submode=togglebanner&bannerid=' . $row['banner_id']),
							'BANNER_NAME'			=> '<a href="' . append_sid('index.php?i=' . $id . '&mode=nwobannerimages&submode=editbanner&bannerid=' . $row['banner_id']) . '" class="title">' . $row['banner_name'] . '</a>',
							'BANNER_IMAGE'			=> get_banner_image($row['banner_image']),
							'BANNER_TYPE'			=> 0,
							'BANNER_LINK'			=> $row['banner_linktarget'],
							'BANNER_DESC'			=> $row['banner_description'],
							'S_BANNER_VISIBLE'		=> $row['banner_visible'] ? true : false,
							'BANNER_SUBMIT'			=> append_sid('index.php?i=' . $id . '&mode=nwobannerimages&submode=addbanner'),
						));
					}
					$db->sql_freeresult($result);
					...
					$sql = 'SELECT COUNT(banner_id) AS banner_count FROM ' . NWOBANNERIMAGES_TABLE;
					$result = $db->sql_query($sql);
					$total_banners = $db->sql_fetchfield('banner_count');
					$db->sql_freeresult($result);

					$action = $this->u_action;
					$template->assign_vars(array(
	    				'PAGINATION'		=> generate_pagination($action, $total_banners, $limit, $start),
		    			'PAGE_NUMBER'		=> on_page($total_banners, $limit, $start),
	    				'TOTAL_BANNERS'		=> $total_banners,
					));

					$this->tpl_name = 'acp_nwomanager_bannerimages';
					$this->page_title = 'ACP_NWOMANAGER_BANNERIMAGES';
				}
			break;
			...
			case 'nwologlist':
				...
				$logserverfilterarray  = array();
				$logserverfilterarray[-4] = 'Website Server';
				$logserverfilterarray[-3] = 'Teamspeak Server';
				$logserverfilterarray[-2] = 'Ventrolli Server';
				$logserverfilterarray[-1] = 'All Game Servers';
				$logserverfilterarray[0] = '- All Servers -';
				...
				while ($rowservers = $db->sql_fetchrow($resultservers))
				{
					...
					$logserverfilterarray[$rowservers['server_id']] = $rowservers['server_name'];
				}
...
/*				$sqlusers = 'SELECT DISTINCT user_id, username
				...
				else
				{
					...
					if (!empty($dofilter))
					{
						$logfilterserver = (int) request_var('logfilterserver', 0);
					...
					if ($logfilterserver == 0 && $logfiltercategory == 0 && $logfiltertype == 0) // reset or default
					{
						$sql_where = '';
					}
					elseif ($logfilterserver != 0 && $logfiltercategory == 0  && $logfiltertype == 0) // server filter used
					{
						$sql_where = " WHERE log_server=$logfilterserver";
					}
					elseif ($logfilterserver == 0 && $logfiltercategory != 0 && $logfiltertype == 0) // category filter used
					{
						$sql_where = " WHERE log_category=$logfiltercategory";
					}
					elseif ($logfilterserver == 0 && $logfiltercategory == 0  && $logfiltertype != 0) // type filter used
					{
						$sql_where = " WHERE log_type=$logfiltertype";
					}
					elseif ($logfilterserver != 0 && $logfiltercategory != 0 && $logfiltertype == 0) // server and category filter used
					{
						$sql_where = " WHERE log_server=$logfilterserver AND log_category=$logfiltercategory";
					}
					elseif ($logfilterserver != 0 && $logfiltercategory == 0  && $logfiltertype != 0) // server and type filter used
					{
						$sql_where = " WHERE log_server=$logfilterserver AND log_type=$logfiltertype";
					}
					elseif ($logfilterserver != 0 && $logfiltercategory != 0  && $logfiltertype != 0) // server, category and type filter used
					{
						$sql_where =  " WHERE log_server=$logfilterserver AND log_category=$logfiltercategory AND log_type=$logfiltertype";
					}
					elseif ($logfilterserver == 0 && $logfiltercategory != 0  && $logfiltertype != 0) // category and type filter used
					{
						$sql_where =  " WHERE log_category=$logfiltercategory AND log_type=$logfiltertype";
					}
					...
					$sql = 'SELECT COUNT(log_id) AS log_count FROM ' . NWOLOGS_TABLE . $sql_where;
					$result = $db->sql_query($sql);
					...
					$template->assign_vars(array(
	    				...
	    				'LOG_FILTERSERVER'	=> build_options_select($logserverfilterarray, !empty($logfilterserver) ? $logfilterserver : 0),
			...
			case 'nwosettings':
				$display_vars = array(
				    ...
				    'vars'    => array(
				        ...
				        'nwomanager_news_maxdisplayed'			=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_NEWS_MAX', 'validate' => 'string', 'type' => 'text', 'explain' => true),

				        'legend2'								=> 'ACP_NWOMANAGER_SETTINGS_TOPICS',
						'nwomanager_topics_forumid'				=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_TOPICS_FORUMID', 'validate' => 'string', 'type' => 'text', 'explain' => true),
						...
						'nwomanager_topics_maxdisplayed'		=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_TOPICS_MAX', 'validate' => 'string', 'type' => 'text', 'explain' => true),
						...
						'legend3'								=> 'ACP_NWOMANAGER_SETTINGS_LOG',
						'nwomanager_log_admins'					=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_LOG_ADMINS', 'validate' => 'string', 'type' => 'text', 'explain' => true),

						'legend4'								=> 'ACP_NWOMANAGER_SETTINGS_BANNERIMAGES',
						'nwomanager_bannerimages_enabled'		=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_BANNERIMAGES_ENABLED', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),
						'nwomanager_bannerimages_folder'		=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_BANNERIMAGES_FOLDER', 'validate' => 'string', 'type' => 'text', 'explain' => true),
						'nwomanager_bannerimages_height'		=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_BANNERIMAGES_HEIGHT', 'validate' => 'int', 'type' => 'text:5:4', 'explain' => true),
						'nwomanager_bannerimages_width'			=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_BANNERIMAGES_WIDTH', 'validate' => 'int', 'type' => 'text:5:4', 'explain' => true),
						'nwomanager_bannerimages_delay'			=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_BANNERIMAGES_DELAY', 'validate' => 'int', 'type' => 'text:5:4', 'explain' => true),
						'nwomanager_bannerimages_fade'			=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_BANNERIMAGES_FADE', 'validate' => 'int', 'type' => 'text:5:4', 'explain' => true),
						'nwomanager_bannerimages_reveal'		=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_BANNERIMAGES_REVEAL', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),

						'legend5'								=> 'ACP_NWOMANAGER_SETTINGS_DONATIONS',
						'nwomanager_donations_target'			=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_DONATIONS_TARGET', 'validate' => 'int', 'type' => 'text:5:4', 'explain' => true),
						'nwomanager_donations_cycle'			=> array('lang' => 'ACP_NWOMANAGER_SETTINGS_DONATIONS_CYCLE', 'validate' => 'int', 'type' => 'text:3:2', 'explain' => true),
				    ),
				);
...
/**********************************************************
 * Gets the image for the banner into <img> tag format, if
 * the image file path is valid otherwise returns ''
 **********************************************************/
function get_banner_image($bannerimageurl)
{
	global $template, $config, $phpbb_root_path;

	$path = $phpbb_root_path . $config['nwomanager_bannerimages_folder'] . '/';

	//echo $path;

	if (!file_exists($path) || !is_dir($path))
	{
		echo 'no file/folder1';
		return "";
	}
	else
	{
		$imageurl = $path . $bannerimageurl;
		...
		else
		{
			...
			return '<img src="' . (str_replace(' ', '%20', $imageurl)) . '" alt="" />';
		}
	}
}
