...
/**********************************************************
 * Main ACP Class for AFK Manager contains:
 * - AFK Manager User AFK List
...
/*				$sql = 'SELECT config_name, config_value
				...
				while($row = $db->sql_fetchrow($result, $limit, $start))
				{
					...
					$afktime = ucwords(afktime(date('Y-m-d H:m:s',strtotime($row['user_afkdate']))));
				...
				$sql = 'SELECT COUNT(user_id) AS afk_status FROM ' . USERS_TABLE . ' WHERE user_afkstatus=' .'"1"';
				$result = $db->sql_query($sql);
				$total_afkstatus = $db->sql_fetchfield('afk_status');
				$db->sql_freeresult($result);

				$action = $this->u_action;
				$template->assign_vars(array(
    				'PAGINATION'		=> generate_pagination($action, $total_afkstatus, $limit, $start),
	    			'PAGE_NUMBER'		=> on_page($total_afkstatus, $limit, $start),
    				'TOTAL_AFKSTATUS'	=> $total_afkstatus,
				));
			...
			case 'afksettings':
				...
				$display_vars = array(
				    ...
				    'vars'    => array(
				        ...
				        'afkmanager_afk_pmmsg_enable'			=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_PMMSG_ENABLE', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),
				        ...
				        'afkmanager_afk_posting_default'		=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_POSTING_DEFAULT', 'validate' => 'string', 'type' => 'textarea', 'explain' => true),
				        ...
				        'afkmanager_afk_viewtopic'				=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_VIEWTOPIC_ENABLE', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),
				        'afkmanager_afk_viewtopic_reason'		=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_VIEWTOPIC_REASON', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),
				        'afkmanager_afk_viewtopic_link'			=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_VIEWTOPIC_LINK', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),
						'afkmanager_afk_viewprofile'			=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_VIEWPROFILE_ENABLE', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),
						'afkmanager_afk_reminder_board'			=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_REMINDER_BOARD', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),
						'legend3'								=> 'ACP_AFKMANAGER_SETTINGS_AUTOAFK',
						'afkmanager_afk_autoafk'				=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_AUTOAFK_ENABLE', 'validate' => 'bool', 'type' => 'radio:yes_no', 'explain' => true),
						'afkmanager_afk_autoafk_default'		=> array('lang' => 'ACP_AFKMANAGER_SETTINGS_AUTOAFK_DEFAULT', 'validate' => 'string', 'type' => 'text', 'explain' => true),
...
/**********************************************************
 * Calculate amount of time user is afk for AFK User List
 * @param $d: date to check against todays date
 * Returns: amount of days, hours or minutes afk
 **********************************************************/
function afktime($d)
{
	$ts = time() - strtotime(str_replace("-","/",$d));
	//$val = round($ts/86400,0).' day';
	if($ts>86400) $val = round($ts/86400,0).' day';
	else if($ts>3600) $val = round($ts/3600,0).' hour';
	else $val = round($ts/60,0).' minute';
	if($val>1) $val .= 's';
	if($val<0) $val = '< 1 minute';
	return $val;
}
