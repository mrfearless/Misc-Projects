...
/**********************************************************
 * Main UCP class for AFK Manager contains:
 * - AFK Manager UCP Module
 **********************************************************/
class ucp_afkmanager
{
	...
	function main($id, $mode)
	{
		...
		$submit = request_var('ucp-afkmanager-status', false);
		...
		if ($submit)
		{
			...
			$afkpmmsg = utf8_normalize_nfc(request_var('AFKPMMSG', (!empty($config['afkmanager_afk_posting_default']) ? $config['afkmanager_afk_posting_default'] : 'My status is marked as Away From Keyboard (AFK) at the moment. Any PMs sent to me in the meantime wont be read until my return.'), true));
			$afkreason = utf8_normalize_nfc(request_var('AFKREASON', '', true));
			$afkreasoncategory = request_var('afkreasoncategoryselect', 0);
			$afkautodays = request_var('AFKAUTODAYS', 0);

			$sql = 'UPDATE ' . USERS_TABLE . '
					SET
					user_afkstatus 		= "' . $afkstatus . '",
					user_afkdate 		= "' . $afkdate . '",
					user_afkautodays	= "' . $afkautodays . '",
					user_afkpmmsg 		= "' . $afkpmmsg . '",
					user_afkreason		= "' . $afkreason . '",
					user_afkreasoncat 	= "' . $afkreasoncategory . '"
			...
			if ($afkmanager_afk_posting_enable == 1) // Has admin enabled for auto posting? if so we post msg
			{
				...
				// If afkstatus is 1, and auto post is enabled, and topic id is 0 we will show the afker their post link as well.
				// If they are setting afkstatus back to 0, we dont show it.
				if ($afkstatus == 1)
				{
					// Check to prevent users repeatedly hitting yes and submitting in UCP, post msg if no topic id or if set to AutoAFK and submit again.
					if ($topicid == 0 && $afkreasoncategory != 99) // Also we check for 99 which is AutoAFK in afkreasonsarray, otherwise a post can be made.
					{
						$template->assign_vars(array(
							'S_AUTO_POST'	=> $config['afkmanager_afk_posting_enable'] ? true : false,
						));
						afkmsg_post('post', 0, $afkreasoncategory, $afkreason, $afkpmmsg);
					}
					else
					{
						$template->assign_vars(array(
				...
				else
				{
					...
					if ($topicid != 0) // Check to prevent users repeatedly hitting no and submitting in UCP. post reply only if a topic id exists
					{
						...
						afkmsg_post('reply', $topicid, 0, '', '');
					}
				}
			}
		}

		$sql = "SELECT user_afkstatus, user_afkdate, user_afkpmmsg, user_afktopicid, user_afkreason, user_afkreasoncat, user_afkautodays
			FROM ". USERS_TABLE ."
			WHERE user_id=". $user->data['user_id'];
		$results = $db->sql_query($sql);
		$row = $db->sql_fetchrow($results);
		...
		$afkreasoncategories = afkreasons_select($afkreasoncategoriesarray, ($row['user_afkstatus'] ? $row['user_afkreasoncat'] : 0));
		$template->assign_vars(array(
			//'S_AFKAUTOPOSTING'		=> ($afkmanager_afk_posting_enable == 0) ? true : false,
			'AFKREASONCATEGORIES'		=> $afkreasoncategories,
		));
		$afkreason = $row['user_afkstatus'] ? $row['user_afkreason'] : '';

		$template->assign_vars(array(
			'AFKSTATUS_PROCESSOR'	=> $this->u_action,
			'AFKSTATUS'				=> $row['user_afkstatus'],
			'AFKDATE'				=> $row['user_afkdate'],
			'AFKAUTODAYS'			=> $row['user_afkautodays'],
...
/**********************************************************
 * Create the AFK post or post replay in specified forum
 * if $mode = 'post' we start a new topic
 * if $mode = 'reply' and we have the AFK users topic id
 * and Auto Post Reply setting is on in ACP we reply to
 * existing AFK post, otherwise we create a new post to
 * tell all users that the AFK user is now back.
 **********************************************************/
function afkmsg_post($mode, $topicid, $afkreasoncategory, $afkreason, $afkmsg)
{
	...
	$topicicon = $config['afkmanager_afk_posting_topicicon'];
	...
	{
		$my_afkmsg = $afkmsg;
		$my_afkmsg .= !empty($config['afkmanager_afk_posting_footer']) ? html_entity_decode($config['afkmanager_afk_posting_footer']) : '';

		if (!empty($afkreason))
		{
			$my_subject = 'My AFK Status: '. $afkreason;
		}
		else
		{
			$my_subject = 'My AFK Status: ';
			$my_subject .= ($afkreasoncategory > 0) ? $afkreasoncategoriesarray[$afkreasoncategory] : 'AFK';
			//$my_subject .= ($afkreasoncategory > 0) ? '[' . $afkreasoncategoriesarray[$afkreasoncategory] . ']' : '[AFK]';
		}
	}
	else
	{
		...
		$my_afkmsg = "Just a quick note to let everyone know I've returned.";
	}

	// Prepare post info
	$subject = utf8_normalize_nfc($my_subject, true);
	$message = utf8_normalize_nfc($my_afkmsg, true);
	...
	$data = array(
	    ...
	    'icon_id'           => $topicicon,    // The Icon ID in which the post will be displayed with on the viewforum, set to false for icon_id. (int)
...
/**********************************************************
 * Create AFK Reasons Categories listbox from array, and set
 * the selected option as well
 **********************************************************/
