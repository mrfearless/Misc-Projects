...
/*			if ($afkstatus == 0)
			...
			if ($afkmanager_afk_posting_enable == 1) // Has admin enabled for auto posting? if so we post msg
			{
				...
				if ($afkstatus == 1)
				{
					if ($topicid == 0) // Check to prevent users repeatedly hitting yes and submitting in UCP, post msg if no topic id 
					{
						...
						postafkmsg('post', 0, $afkreasoncategory, $afkreason, $afkpmmsg);
					}
					else 
					{
						$template->assign_vars(array(
		...
		$sql = "SELECT user_afkstatus, user_afkdate, user_afkpmmsg, user_afktopicid, user_afkreason, user_afkreasoncat
			FROM ". USERS_TABLE ."
			WHERE user_id=". $user->data['user_id'];
		$results = $db->sql_query($sql);
		$row = $db->sql_fetchrow($results);
...
function postafkmsg($mode, $topicid, $afkreasoncategory, $afkreason, $afkmsg)
{
	...
	{	
		$my_afkmsg = $afkmsg;
		$my_afkmsg .= !empty($config['afkmanager_afk_posting_footer']) ? $config['afkmanager_afk_posting_footer'] : '';
		
		if (!empty($afkreason))
		{
			$my_subject = 'AFK '. $afkreason;
		}
		else 
		{
